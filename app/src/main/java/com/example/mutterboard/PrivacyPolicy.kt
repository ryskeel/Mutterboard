 ...
 */
const val PRIVACY_POLICY_UPDATED = "Last updated: 2 July 2026"
...
• Default mode (cloud): your recorded audio is sent over an encrypted (HTTPS) connection to Groq (api.groq.com) to be transcribed by the Whisper model. Your Groq API key is sent with the request to authenticate it. Groq's handling of that audio is governed by Groq's own privacy policy at groq.com/privacy-policy. Mutterboard does not keep a copy of the audio after transcription.

• Offline mode (on-device): your audio is transcribed locally on your phone using the Parakeet model. In this mode your audio never leaves your device.
...
The transcribed text is inserted into the app you are typing in. Mutterboard does not store or log your transcribed text, and does not transmit it anywhere except as described under "Cleanup step" below. As a keyboard, Mutterboard does not capture your keystrokes and does not read the existing contents of the text fields you use it in.

CLEANUP STEP (DEFAULT MODE)
In Default mode, after your speech is transcribed by Whisper, the transcribed text is sent over an encrypted (HTTPS) connection to Groq (api.groq.com) to be lightly cleaned up by Alibaba's Qwen3.6 language model (which tidies the phrasing and punctuation) before it is typed out. Your Groq API key is sent with the request to authenticate it. Groq's handling of that text is governed by Groq's own privacy policy at groq.com/privacy-policy. Mutterboard does not keep a copy of the text after the cleanup. In Offline mode, your transcribed text is never sent anywhere.
...
NETWORK CONNECTIONS
Mutterboard makes network connections only for: (1) cloud transcription, which sends audio to Groq, in Default mode only; (2) the cleanup step, which sends your transcribed text to Groq, in Default mode only; (3) checking for app updates from GitHub; and (4) a one-time speech-model download from GitHub if you enable Offline mode. These requests carry no personal information beyond what is described above.
